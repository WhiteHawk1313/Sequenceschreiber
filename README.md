...
Open the workbook. The Workbook_Open event initializes dropdown lists and device settings.
Select a method in the dropdown (cell I3). The worksheet will automatically:
...
Changes sample weighings in column C will trigger recalculation of weights.
...
Provides utility functions and enumerations for handling sequence properties and measurement types.
    ...
    funcGetMethode(strTopic As Variant, Metadaten As Object):  Returns method of requestes product.
    ...
    funcGetMaxPosition(colFinalSequence): Returns highes positon value in given sequence.
    funcHasIntermediataCalibration(colSequence): Returns boolean value if sequecne has an intermediata calibration.
...
8. Process Each Sample in Export
    For each sample:
        Replace commas with dots in the weighings.
        ...
        Check if the sample is a leader sample:
            ...
            If False:
                Sum all weighings for the sample.
